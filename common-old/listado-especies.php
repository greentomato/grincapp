<!-- LOCATION -->

<div class="smallNav">
    <a href="?view=2"><i class="fa fa-arrow-left"></i>Volver</a>
</div>
<div class="nativaListHeader">
    <a href="#" class="asideItemHeader">Especies nativas en</a>
    <h2><span>Jard&iacute;n contrafrente</span></h2>
<!--  <input type="text" placeholder="Buscar por nombre"> -->
</div>


<div class="hr"></div>


<!-- SPECIES LIST -->
<div class="nativaList">
    <div class="nativaListItem">
        <a href="?view=4">
            <img src="img/nativas/balcon/a.jpg" alt="Nativa">
            <div class="nativaListItemTitle">
                <h3>Ceibo</h3>
                <p>Lorem ipsum dolor</p>
            </div>
        </a>
    </div>
    
    <div class="nativaListItem">
        <a href="?view=4">
            <img src="img/nativas/balcon/b.jpg" alt="Nativa">
            <div class="nativaListItemTitle">
                <h3>Lorem</h3>
                <p>Lorem ipsum dolor</p>
            </div>
        </a>
    </div>

    <div class="nativaListItem">
        <a href="?view=4">
            <img src="img/nativas/balcon/c.jpg" alt="Nativa">
            <div class="nativaListItemTitle">
                <h3>Ipsum</h3>
                <p>Lorem ipsum dolor</p>
            </div>
        </a>
    </div>

    <div class="nativaListItem">
        <a href="?view=4">
            <img src="img/nativas/balcon/d.jpg" alt="Nativa">
            <div class="nativaListItemTitle">
                <h3>Dolor</h3>
                <p>Lorem ipsum dolor</p>
            </div>
        </a>
    </div>

    <div class="nativaListItem">
        <a href="?view=4">
            <img src="img/nativas/balcon/e.jpg" alt="Nativa">
            <div class="nativaListItemTitle">
                <h3>Amet</h3>
                <p>Lorem ipsum dolor</p>
            </div>
        </a>
    </div>

    <div class="nativaListItem">
        <a href="?view=4">
            <img src="img/nativas/balcon/f.jpg" alt="Nativa">
            <div class="nativaListItemTitle">
                <h3>Nomen</h3>
                <p>Lorem ipsum dolor</p>
            </div>
        </a>
    </div>

    <div class="nativaListItem">
        <a href="?view=4">
            <img src="img/nativas/balcon/g.jpg" alt="Nativa">
            <div class="nativaListItemTitle">
                <h3>Dosis</h3>
                <p>Lorem ipsum dolor</p>
            </div>
        </a>
    </div>

    <div class="nativaListItem">
        <a href="?view=4">
            <img src="img/nativas/balcon/h.jpg" alt="Nativa">
            <div class="nativaListItemTitle">
                <h3>Suxit</h3>
                <p>Lorem ipsum dolor</p>
            </div>
        </a>
    </div>

</div>






